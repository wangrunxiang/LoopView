 ...
 */
package com.wangrunxiang.loopview.widget.viewPagerIndicator;
...
import com.wangrunxiang.loopview.R;
...
public class CirclePageIndicator extends View implements PageIndicator {
	...
	public CirclePageIndicator(Context context, AttributeSet attrs, int defStyle) {
		...
		mCentered = a.getBoolean(R.styleable.CirclePageIndicator_indicator_centered,
				defaultCentered);
		...
		mRadius = a.getDimension(R.styleable.CirclePageIndicator_indicator_radius,
				defaultRadius);
		final float defaultGap = 3 * mRadius;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			float drawLong = longOffset + (iLoop * gap);
			if (mOrientation == HORIZONTAL) {
				dX = drawLong;
				dY = shortOffset;
			} else {
		...
		float cx = (mSnap ? mSnapPage : mCurrentPage) * gap;
		if (!mSnap) {
			cx += mPageOffset * gap;
		}
		if (mOrientation == HORIZONTAL) {
